...
- (NSString *)cacheKeyForURL:(NSURL *)url andDictionary:(NSDictionary *) info
{
    ...
    if (transformation)
    {
        ...
        else
        {
            ...
            else if ([transformation isEqualToString:@"blur"])
            {
              cacheKey = [NSString stringWithFormat:@"%@_blur_%@", [self cacheKeyForURL:url], [info objectForKey:@"blur"]];
            }
        }
    }
    return cacheKey;
...
- (void)downloadWithURL:(NSURL *)url delegate:(id<SDWebImageManagerDelegate>)delegate options:(SDWebImageOptions)options userInfo:(NSDictionary *)userInfo
{
    ...
    NSArray *transformationKeys = [NSArray arrayWithObjects:@"transformation", @"bounds", @"size", @"content_mode", @"bias", nil];
...
- (void)imageDownloader:(SDWebImageDownloader *)downloader didFinishWithImage:(UIImage *)image
{
    ...
    else
    {
        ...
        else if ([[downloader.userInfo objectForKey:@"transformation"] isEqual: @"blur"])
        {
          NSInteger bias = [[downloader.userInfo objectForKey:@"bias"] intValue];
          image = [image gaussianBlurWithBias:bias];
          downloader.imageData = [UIImagePNGRepresentation(image) copy];
        }
    }
